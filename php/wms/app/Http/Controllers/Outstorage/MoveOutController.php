...
class MoveOutController extends OutBaseController
{
    protected $rules = [];
    ...
    public function index(Request $request)
    {
        ...
        if ($request->get('MoveNo')) $params[] = ['o.MoveNo', 'like', '%' . $request->get('MoveNo')];
        ...
        if ($request->get('MovFrmCD')) $params[] = ['o.MovFrmCD', 'like', '%' . $request->get('MovFrmCD')];
        if ($request->get('EMPST')) $params[] = ['o.EMPST', 'like', '%' . $request->get('EMPST')];
        if ($request->get('zt')) $params[] = ['o.zt', 'like', '%' . $request->get('zt')];
        if ($request->get('MovToCD')) $params[] = ['o.MovToCD', 'like', '%' . $request->get('MovToCD')];
        ...
        $builder = DB::table('move_out_dirt as o')
            ->where($params)
            ->select(
                'o.MoveNo',
                'o.MovFrmCD',
                ...
                DB::raw('sum(o.MovAdmQnty) as number')
            )
            ->orderBy('created_at', 'desc')
            ->groupBy('o.MoveNo', 'o.EMPST', 'o.MovFrmCD', 'o.AUART', 'o.MovToCD', 'o.OutStcNo', 'o.FineFlg', 't.status','o.time','o.zt')
            ->leftJoin('move_out_dirt_tag as t', 'o.id', '=', 't.related_id');
    ...
    public function getByNo(Request $request)
    {
        ...
        $orders = MoveOut::where('OutStcNo', $outStcNo)->with(['product', 'deliver', 'tag'])->get();
    ...
    public function stockOut(Request $request)
    {
        ...
        if ($all['status'] == '复核中') {
            foreach ($all['orders'] as $key => $val) {
                $list = MoveOutTag::where('related_id', $val['id'])->get();
        ...
        //     //记录日志
        //     Log::info('操作人：'.$username.'出库类型：move_out_dirt'.'出库单号：'.$all['orders'][0]['OutStcNo']);
        //     $this->moveByNo($all['orders'][0]['OutStcNo']);
        //     return sendData();
        // }
        $error = $this->outStorage($all, new MoveOutTag, 'move_out_dirt', $all['orders'][0]['MoveNo']);
        if($all['status'] == '拣货中'){
    ...
    public function backNo(Request $request)
    {
        ...
        $time = time();
        $arr = MoveOut::where('OutStcNo',$OutStcNo)->first();
        if(!$arr){
        ...
        Log::useFiles(storage_path('logs/Outreturn/error' . $name . '.log'));
        //记录日志
        Log::info('操作人：' . $username . '出库类型：move_out_dirt' . '出库单号：' . $OutStcNo);
        $this->moveByNo($OutStcNo,$time);
        return sendData();
    }
    ...
    public function exportDoc(Request $request)
    {
        ...
        $customer = MoveOut::where('MoveNo', $OutStcNo)->with(['deliver'])->get();
        ...
        if(!$customer[0]->deliver){
             $res = [['拣货单号:', '', '', '发票号:', $OutStcNo, '', '交货单号:', $customer[0]->OutStcNo], '', ['客户名: ' .'', '', '', '客户号:', '', '', '拣货日期:', $time], ['拣货明细:'], ['品名', '新产品代码', '商品代码', '防串货', '有效期', '单位', '数量', '余量', '箱数', '库位号', '备注']];
        }else{
            $res = [['拣货单号:', '', '', '发票号:', $OutStcNo, '', '交货单号:', $customer[0]->OutStcNo], '', ['客户名: ' . $customer[0]->deliver->DeliverAddNM, '', '', '客户号:', '', '', '拣货日期:', $time], ['拣货明细:'], ['品名', '新产品代码', '商品代码', '防串货', '有效期', '单位', '数量', '余量', '箱数', '库位号', '备注']];
        }
        // 最后一条特殊处理
        $prf[] = $pre[$pre_key];
    ...
    public function exportPDF(Request $request)
    {
        $OutStcNo = $request->get('id');
        if (!$OutStcNo) return sendData(402, 'ID为空！');
        $salesOuts = MoveOut::where('OutStcNo', $OutStcNo)->with(['company', 'deliver', 'product'])->get();
    ...
    public function accept(Request $request)
    {
        ...
        $txt = "地址：" . $salesOuts[0]->deliver
            . "\n邮编：" . $salesOuts[0]->deliver
            . "\n电话：" . $salesOuts[0]->deliver
            . "\nFAX：" . $salesOuts[0]->deliver
            . "\n\n订单原因："
            . "\n\n特殊送货地址：" . $salesOuts[0]->DeliverAddMemo
            . "\n备注：" . $salesOuts[0]->Memo
            . "\nPO编号：" . $salesOuts[0]->ApplyNo;
        ...
        $data[] = ['', '', '', '合计', $totalNum, $totalRISETAXMNY];
        $pdf->getTable($data);
        $filename = storage_path() . '/' . $OutStcNo . '.pdf';
        //输出PDF
        $pdf->Output($filename, 'I');
    ...
    public function rollback(Request $request)
    {
        ...
        $ganher = DB::table('ganher')->where('vbeln', $outStcNo['orders'][0]['MoveNo'])->get();
        try {
            DB::beginTransaction();
            if (count($ganher) != 0) {
                if ($ganher[0]->status == '未集货'|| $ganher[0]->status == 'PC' || $ganher[0]->status == 'App') {
                    foreach ($outStcNo as $key => $val) {
                        foreach ($val as $item) {
                            ...
                            $goods_id = DB::table('goods_tag')->where('odd',$item['MoveNo'])->pluck('goods_id')->toArray();
                            Goods::whereIn('id',$goods_id)->delete();
                            DB::table('goods_tag')->where('odd',$item['MoveNo'])->delete();
                            $id = [
                                'id' => $item['tag']['id'],
                            ];
                            DB::table('move_out_dirt_tag')->where($id)->delete();
                        }
                    }
                    DB::table('ganher')->where('vbeln', $outStcNo['orders'][0]['MoveNo'])->delete();
                    Redis::del('jianhuo::' . $outStcNo['orders'][0]['MoveNo']);
                    DB::table('out_record')->where('odd',$outStcNo['orders'][0]['MoveNo'])->update(['deleted_at'=>date('Y-m-d H:i:s')]);
                } else {
    ...
    public function export(Request $request)
    {
        ...
        if ($parm['id']) {
            ...
            $salesOuts = MoveOut::whereIn('MoveNo', $pieces)->with(['tag'])->get();
        } else {
            ...
            if (isset($querys->MovToCD)) $params[] = ['MovToCD', 'like', '%' . $querys->MovToCD];
            ...
            $salesOuts = MoveOut::where($params)
        ...
        $data = GoodsRecord::whereIn('related_id', $relatedIds)->where('type', 'move_out_dirt')->with(['product'])->get();
        $res = [['品名', '新产品代码', '产品代码', '有效期', '出入库类型', '数量', '状态', '库位号', 'SAP单号', '出库单号', '备注', '创建时间']];
        foreach ($data as $key => $value) {
            $odd = MoveOut::where('id', $value['related_id'])->with(['tag'])->first();
            $res[] = [
...
//                $unit[] = $list[0]->NewPRODUCTCD;
        ...
        $data = DB::table('binning as b')
            ...
            ->orderBy('p.PRODUCTCD', 'asc')
            ->get();
        $ord = DB::table('move_out_dirt as o')
            ->where('o.MoveNo', $data['0']->vbeln)
            ->get();
        $res = [['资生堂(中国)投资有限公司'], [], ['送货地代码:', $ord[0]->DeliverAddCD], ['客户单号:', $data['0']->vbeln], ['箱号', '', '商品代码', '效期', '产品名称', '箱数', '数量']];
        $result = [];
        foreach ($data as $key => $value) {
...
//            if (in_array($value->case, $exist)) {
//                if (!in_array($value->case, $data)) {
//                    $data[] = $value->case;
//                }
//            } else {
//                $exist[] = $value->case;
...
//        $data = array_unique($exist);
    ...
    public function downloadPX(Request $request)
    {
        ...
        $ord = DB::table('move_out_dirt')
            ->where('MoveNo', $data['0']->vbeln)
            ->get();
        $pre = $prf = [];
        $pre_key = '';
        ...
        // 最后一条特殊处理
        $prf[] = $pre[$pre_key];
        $res = [['送货地代码', $ord[0]->DeliverAddCD, '', '订单号', $data['0']->vbeln], ['箱号', '', $request->case], [], ['商品代码', '有效期', '品名', '数量', '备注']];
        foreach ($prf as $key => $value) {
            $res [] = [
                $value->PRODUCTCD,
                $value->available_time,
...
//                $unit[] = $list[0]->NewPRODUCTCD;
        ...
        Excel::create($name, function ($excel) use ($res) {
            ...
            foreach ($data as $re=>$val) {
                ...
                $prf[] = $pre[$pre_key];
                $ord = DB::table('move_out_dirt')
                    ->where('MoveNo', $row['0']->vbeln)
                    ->get();
                $k = [ ['送货地代码', $ord[0]->DeliverAddCD, '','订单号', $res[3][1]], ['箱号','', $val], [],['商品代码', '有效期', '品名', '数量', '备注']];
                foreach ($prf as $key => $value) {
                    $k [] = [
                        $value->PRODUCTCD,
                        $value->available_time,
