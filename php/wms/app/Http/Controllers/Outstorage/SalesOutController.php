...
class SalesOutController extends OutBaseController
{
    protected $rules = [];
...
//        if ($request->get('print_status')) $params[] = ['o.print_status', 'like', '%' . $request->get('print_status')];
        ...
        $builder = DB::table('ord_out_dirt as o')
            ->select(
                'o.OrderNo',
                'o.OutStcNo',
                'o.AUART',
                ...
                DB::raw('sum(o.AdmQnty) as number')
            )
            ->where($params)
            ->orderBy('created_at', 'desc')
            ->groupBy('o.OrderNo', 'o.OutStcNo', 'o.AUART', 'o.FineFlg', 'c.ShopSignNM', 't.status', 'o.zt', 'o.time', 'o.print_status')
            ->leftJoin('customer as c', 'o.CustomerCd', '=', 'c.CUSTOMERCD')
            ->leftJoin('ord_out_dirt_tag as t', 't.related_id', '=', 'o.id');
    ...
    public function getByNo(Request $request)
    {
        ...
        $orders = SalesOut::where('OutStcNo', $outStcNo)->with(['product', 'customer', 'deliver', 'tag'])->get();
    ...
    public function getBySee(Request $request)
    {
        ...
        $orders = SalesOut::where('OutStcNo', $outStcNo)->with(['product', 'customer', 'deliver', 'tag', 'company'])->get();
        if (count($orders) == 0) return sendData(402, '获取失败');
        return sendData(200, '', $orders);
    }

    public function getByOut(Request $request)
    {
        $all = $request->all();
        $arr = [];
        foreach ($all as $key) {
            $arr[] = [
                'odd' => $key['odd'],
                'NewProductCd' => $key['NewProductCd'],
                'stock_no' => $key['stock_no'],
                'status' => $key['status'],
                'number' => $key['actNumber'],
                'created_at' => date('Y-m-d H:i:s'),
                'user_id' => $this->user->id,
            ];
        }
        DB::table('out_record')->insert($arr);
    ...
    public function stockOut(Request $request)
    {
        ...
        if ($all['status'] == '复核中') {
            foreach ($all['orders'] as $key => $val) {
                $list = SalesOutTag::where('related_id', $val['id'])->get();
        ...
        //     //记录日志
        //     Log::info('操作人：'.$username.'出库类型：ord_out_dirt'.'出库单号：'.$all['orders'][0]['OutStcNo']);
        //     $this->generate($all['orders'][0]['OutStcNo']);
        //     return sendData();
        // }

        $error = $this->outStorage($all, new SalesOutTag, 'ord_out_dirt', $all['orders'][0]['OrderNo']);
        if ($all['status'] == '拣货中') {
    ...
    public function backNo(Request $request)
    {
        ...
        $time = time();
        $flagData = Redis::get('outFlag');
        if ($flagData || $flagData === 1) {
            return sendData(402, '单子生成中，请稍后再试');
        }
        Redis::set('outFlag', 1);
        switch ($type) {
            case 'ord':
                $arr = SalesOut::where('OutStcNo', $OutStcNo)->first();
                if (!$arr) {
                ...
                Log::useFiles(storage_path('logs/Outreturn/error' . $name . '.log'));
                //记录日志
                Log::info('操作人：' . $username . '出库类型：ord_out_dirt' . '出库单号：' . $OutStcNo);
                $this->generate($OutStcNo, $time);
                break;
            case 'adj':
    ...
    public function exportDoc(Request $request)
    {
        ...
        $customer = SalesOut::where('OrderNo', $OutStcNo)->with(['customer'])->get();
    ...
    public function exportPDF(Request $request)
    {
        $OutStcNo = $request->get('id');
        if (!$OutStcNo) return sendData(402, 'ID为空！');
        $salesOuts = SalesOut::where('OutStcNo', $OutStcNo)->with(['company', 'deliver', 'customer', 'product'])->get();
        ...
        foreach ($salesOuts as $salesOut) {
            ...
            $temp[] = $salesOut->AdmQnty;
            $temp[] = $salesOut->RPRICEMNY;
            $temp[] = $salesOut->WPRICEMNY;
            $temp[] = $salesOut->WHOLEMNY;
            $temp[] = $salesOut->UPRICEMNY;
            $temp[] = $salesOut->UNITMNY;
            $temp[] = $salesOut->RISETAXMNY;
            $data[] = $temp;
            $count++;
            $totalNum += $salesOut->AdmQnty;
    ...
    public function rollback(Request $request)
    {
        ...
        $ganher = DB::table('ganher')->where('vbeln', $outStcNo['orders'][0]['OrderNo'])->get();
                ...
                if ($ganher[0]->status == '未集货' || $ganher[0]->status == 'PC' || $ganher[0]->status == 'App') {
                    foreach ($outStcNo as $key => $val) {
                        foreach ($val as $item) {
                            ...
                            $goods_id = DB::table('goods_tag')->where('odd', $item['OrderNo'])->pluck('goods_id')->toArray();
                            Goods::whereIn('id', $goods_id)->delete();
                            DB::table('goods_tag')->where('odd', $item['OrderNo'])->delete();
                            $id = [
                                'id' => $item['tag']['id'],
                            ];
                            DB::table('ord_out_dirt_tag')->where($id)->delete();
                        }
                    }
                    DB::table('ganher')->where('vbeln', $outStcNo['orders'][0]['OrderNo'])->delete();
                    Redis::del('jianhuo::' . $outStcNo['orders'][0]['OrderNo']);
                    DB::table('out_record')->where('odd', $outStcNo['orders'][0]['OrderNo'])->update(['deleted_at' => date('Y-m-d H:i:s')]);
                } else {
    ...
    public function wave(Request $request)
    {
        ...
        $salesOuts = SalesOut::whereIn('OrderNo', $pieces)->with(['customer', 'tag'])->get();
        ...
        $records = GoodsRecord::where('type', 'ord_out_dirt')
        ...
        foreach ($records as $v) {
            $OrderNo = SalesOut::where('id', $v->related_id)->get();
            $where = [
                'product_id' => $v->product_id,
                'unit_name' => "箱"
            ];
    ...
    public function export(Request $request)
    {
        ...
        if ($parm['id']) {
            ...
            $salesOuts = SalesOut::where($params)
        ...
        $data = GoodsRecord::whereIn('related_id', $relatedIds)->where('type', 'ord_out_dirt')->with(['product'])->get();
        $res = [['品名', '新产品代码', '产品代码', '客户代码', '客户名', '有效期', '出入库类型', '数量', '状态', '库位号', 'SAP单号', '出库单号', '备注', '创建时间']];
        foreach ($data as $key => $value) {
            $odd = SalesOut::where('id', $value['related_id'])->with(['customer', 'tag'])->first();
            $res[] = [
    ...
    public function downloadPXHZ(Request $request)
    {
        ...
        Excel::create($name, function ($excel) use ($res, $all) {
            ...
            DB::table('ord_out_dirt')->where('OrderNo', $all['id'])->update(['print_status' => '已打印']);
        })->export('xls');
    }
}
