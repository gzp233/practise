<template>
    <div class="grid-container">
        <div class="grid-toolbar">
            ...
            <button class="tool-btn btn-add" @click="shopping">添加到购物车</button>
            <button class="tool-btn btn-add" @click="car">购物车</button>
        </div>
        ...
        <div class="grid-content">
            <el-table
                ...
                <el-table-column
                        ...
                        width="150"
                ></el-table-column>
                <el-table-column prop="number" label="可用数量" width="100"></el-table-column>
...
<script>
    import { goodsList, getGoodsByProductId, getByNo, relieve ,getDetailById ,shopping , shoppingIndex,getDetailByIds} from "@/api/check";
    ...
    export default {
        ...
        methods: {
            ...
            car(){
                this.$router.push({
                    path: "/storage_action/check/shopping/"+this.$route.params.id
                });
                this.loadData(this.$route.params.id);
            },
            Shelf() {
                this.$router.push({
                    path: "/storage_action/check/list/"+this.$route.params.id
                });
                this.loadData(this.$route.params.id);
            },
            shopping(){
                ...
                var goodsIdsList = [];
                if (this.checked.length === 0 && !this.query.BRANDNM && !this.query.NewPRODUCTCD && !this.query.PRODCHINM && !this.query.PRODFLGNM && !this.query.available_time && !this.query.state_name && !this.query.stock_no) {
                    this.$message({
                        message: "请选择商品！",
                        type: "warning"
                    });
                ...
                if(goodsIdsList.length == 0){
                  shoppingIndex(this.query).then(res => {
            ...
            batches() {
               ...
               if (this.checked.length === 0 && !this.query.BRANDNM && !this.query.NewPRODUCTCD && !this.query.PRODCHINM && !this.query.PRODFLGNM && !this.query.available_time && !this.query.state_name && !this.query.stock_no) {
                ...
                if(goodsIdsList.length == 0){
                  getDetailByIds(this.query).then(res => {
                  console.log(223);
                ...
                this.loadData(this.$route.params.id);
                  ...
                  getDetailById(goodsIdsList).then(res => {
                    goodsIdsList = res.data;
                localStorage.goodsStock_query = JSON.stringify(this.query)
                this.loadData(this.$route.params.id);
                });
