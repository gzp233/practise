...
<script>
  import { getGoodsByProductId, getByNo, relieve,verify,batches} from "@/api/check";
  ...
  export default {
    ...
    methods: {
      ...
      showDetail: function(detail) {
        ...
        this.$router.push({
          path: "/storage_action/check/particulars/"+detail.batches
        });
      },
      ...
      Shelf() {
        ...
        verify(goodsIdsList).then(res => {
          if (res.data) {
          window.open(
                  "/api/check/getExport?name=" + res.data + "&token=" + token[1],
                  "_blank"
          );
        }
