  ...
  <div class="grid-container">
    <div class="page-bread">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>受注出库处理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      <el-tabs activeName="table">
        <el-tab-pane label="商品明细" name="table">
          ...
          >
            ...
            >产品代码：{{ tmpGoods.NewProductCd }}, 出库数量: {{ tmpGoods.AdmQnty }}</p>
...
<script>
import { getByNo, stockOut, rollback, pxList,getByOut } from "@/api/sales_out";
...
export default {
  ...
  methods: {
    ...
    handleChange(row){
       row.actNumber = row.actNumber.replace(/[^\d]/,"");      
    },
    changelog(row){
      this.count = 1;  
    },
    handleChoose(row) {      
      ...
      console.log(row,this.initialize)
      for(let i=0;i<old_data.length;i++){
        let obj = {};
        if(Number(old_data[i].actNumber) != 0){               
          obj.odd = this.order.OrderNo;
          ...
          arr.push(obj); 
          [...this.deepArr] = arr;
          console.log(old_data,arr)
        };
      }; 
      localStorage.setItem("row",JSON.stringify(old_data))                 
    ...
    doChange() {
      ...
      stockOut({
        ...
        .then(res => {
          ...
          this.loadOrder(this.$route.params.id);
          this.$router.push({
            path: "/outstorage_action/sell_out/out/" + this.order.OutStcNo
          });
        })
        .catch(err => {
          this.loadOrder(this.$route.params.id);
    ...
    saveNumber() {
      ...
      if (total != this.tmpGoods.AdmQnty) {
        this.$message({
          message: "出库数量不正确",
          type: "warning"
        });
      ...
      for(let i=0;i<this.tmpGoods.stocks.length;i++){
        if(this.count === 1){
           if(Number(this.tmpGoods.stocks[i].actNumber) != 0){
            ...
            this.arr.push(obj); 
            console.log(this.arr,this.tmpGoods.stocks)         
          }
        }else{
