...
<script>
import { fetchList } from "@/api/move_out";
import { backNo } from "@/api/sales_out";
import { getByNo, moveByNo } from "@/api/out_ensure";
...
export default {
  ...
  created() {
    if (localStorage.moveout_query) {
      this.query = JSON.parse(localStorage.moveout_query);
      delete localStorage.moveout_query;
    }
  ...
  methods: {
    handleComes(row){
       this.disabled = true;
       backNo({OutStcNo: row.OutStcNo,type:'move'}).then(res =>{
          if(res.code === 200){
             this.disabled = false;
    ...
    showDetail: function(detail) {
      localStorage.moveout_query = JSON.stringify(this.query);
      this.$router.push({
        path: "/outstorage_action/move_out/detail/" + detail.OutStcNo
      });
    },
    handleOut(detail) {
      localStorage.moveout_query = JSON.stringify(this.query);
      this.$router.push({
        path: "/outstorage_action/move_out/out/" + detail.OutStcNo
      });
    },
    getRowKeys(row) {
      return row.OutStcNo;
    ...
    exportOut() {
      ...
      window.open(
        "/api/move_out/export?id=" + goodsIdsList + "&query=" + JSON.stringify(this.query) + "&token=" + token[1],
        "_blank"
      );
      localStorage.moveout_query = JSON.stringify(this.query);
      this.loadData();
