    ...
    <div class="detail-content">
      <div class="btn-box">
        ...
        <el-button
          :disabled="buttonDisabled"
          type="primary"
          v-show="!order.tag || order.tag.status !== '发货完成'"
          @click="doChange"
      ...
      <el-tabs activeName="table">
        <el-tab-pane label="商品明细" name="table">
          ...
          >
            ...
            >产品代码：{{ tmpGoods.NewProductCD }}, 出库数量: {{ tmpGoods.MovAdmQnty }}</p>
...
<script>
import { getByNo, stockOut, rollback, pxList } from "@/api/move_out";
...
export default {
  ...
  methods: {
    ...
    doChange() {
      ...
      stockOut({ orders: this.products, status: this.buttonStatus })
        .then(res => {
          if (
            this.order &&
            this.order.tag &&
          ...
          this.loadOrder(this.$route.params.id);
          this.$router.push({
            path: "/outstorage_action/move_out/out/" + this.order.OutStcNo
          });
        })
        .catch(err => {
          this.loadOrder(this.$route.params.id);
    ...
    saveNumber() {
      ...
      if (total != this.tmpGoods.MovAdmQnty) {
        this.$message({
          message: "出库数量不正确",
          type: "warning"
        });
