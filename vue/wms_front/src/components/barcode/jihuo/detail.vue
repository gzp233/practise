    ...
    <div class="detail_content">
      ...
      <div class="show-group">        
        <el-table :data="records" stripe style="width: 100%;" max-height="250">
          ...
          <el-table-column class="button-item" prop="surplus" label="零头" min-width="40"></el-table-column>
          <el-table-column class="button-item" prop="number" label="出库数量" min-width="80"></el-table-column>
          <el-table-column class="button-item" prop="scanNumber" label="扫描" min-width="40"></el-table-column>
        </el-table>
...
import { doJihuoStock, getJihuoStock } from "@/api/jihuo";
...
export default {
  ...
  mounted() {
    getJihuoStock(this.$route.query).then(res => {
  ...
  methods: {
    handleAdd() {
      ...
      if (!this.form.zhima) {
        ...
        if (tit.length == 23) {
          ...
          if (tit.length == 13 || tit.length == 14 || tit.length == 12) {
            for (let i = 0; i < this.records.length; i++) {
              if(this.count == 1){
                 this.form.zhima = tit;
                 this.count = 2;
    ...
    doCreate() {
      doJihuoStock(this.records).then(res => {
        this.$message({
          message: "拣货成功",
          type: "success"
        });
        this.$router.push({
          path: "/barcode/jihuo/stockList/" + this.$route.query.id
        });
      });
    },
    handleBack() {
