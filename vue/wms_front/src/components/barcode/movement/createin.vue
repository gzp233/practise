<template>
  <div class="input-group">
    ...
    <div class="head">
      <span ref="tabTag" style="text-align:center;position: absolute;top:32px;left:10px">入库库位扫码</span>
      <el-input
...
import { getStockListByNo, getToStockList, getGoodsList,submitTo } from "@/api/out";
...
export default {
  ...
  mounted() {
    getToStockList({ "yiku_no": this.$route.query.moveNumber }).then(res => {
  ...
  methods: {
    handleAdd() {   // 扫描结果
        ...
        getGoodsList({"yiku_no":this.$route.query.moveNumber}).then(res => {
            console.log("所有扫入的移出商品列表:" + res);
        });
    },
    handleBack() {  //返回
      this.$router.push({
    ...
    handleyiku() {  // 提交
        if(this.stockList.length >= 1){  // 至少一个已完成
           this.disabled = true;
           submitTo({"yiku_no":this.$route.query.moveNumber}).then(res => {
             if(Object.is(res.code,200)){
                 this.$router.push({
                    path: "/barcode/movement/"
                 });
                 this.$message("移库任务已完成");
             };
           }).catch(res =>{
             this.disabled = false;
           });
