...
import { getStock, doStock } from "@/api/out";
...
export default {
  ...
  mounted() {
    getStock({"yiku_no":this.$route.query.moveNumber,"stock_no":this.$route.query.stock_no,"type":"to"}).then(res => {
  ...
  methods: {    
    ...
    doCreate() {
      ...
      this.objData.type = "to";
      this.disabled = true;
      doStock(this.objData).then(res => {
        if(Object.is(res.code,200)){          
          ...
          this.$router.push({
            ...
            query:{moveNumber:this.$route.query.moveNumber,tag:"in"}
          });
        }
      }).catch(res =>{
        this.disabled = false;
