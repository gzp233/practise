<template>
  <div class="grid-container">
    <div class="search-form">
      <el-form :inline="true" :model="query">
        <el-form-item label="名称">
          <el-input v-model="query.permission_name" placeholder="权限名称"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <div class="grid-toolbar">
      <button class="tool-btn btn-add" @click="add">新增权限</button>
    </div>
    <div class="grid-content">
      <el-table :data="tableData" v-loading.body="loading" stripe height="800" style="width: 100%">
        <el-table-column type="index" label="编号" width="100">
        </el-table-column>
        <el-table-column prop="permission_name" label="权限名称" min-width="250">
        </el-table-column>
        <el-table-column prop="permission_code" label="权限标识" min-width="350">
        </el-table-column>
        <el-table-column prop="desc" label="权限描述" min-width="300">
        </el-table-column>
    ...
    <!-- 详情弹框 -->
    <el-dialog title="权限详情" :visible.sync="dialogVisible" size="dl-small" :modal-append-to-body="false" :close-on-press-escape="false">
...
<script>
import CustomerDetail from '@/components/users/permission/permissionDetail'
import { fetchList, deletePermission } from '@/api/permission'
...
export default {
  ...
  methods: {
    ...
    add: function() {
      this.$router.push({ path: '/users/permission/add' })
    },
    edit: function(row) {
      this.$router.push({
        path: '/users/permission/edit/' + row.id
      })
    },
