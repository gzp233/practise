...
<script>
import { fetchList } from "@/api/procurement_storage";
...
export default {
  ...
  created() {
    if (localStorage.procurement_query) {
      this.query = JSON.parse(localStorage.procurement_query);
      delete localStorage.procurement_query;
    }
  ...
  methods: {
    ...
    showDetail: function(detail) {
      localStorage.procurement_query = JSON.stringify(this.query);
      this.$router.push({
        path: "/instorage_action/procurement_storage/detail/" + detail.InvoiceNo
      });
    },
    getRowKeys(row) {
    ...
    handleStockIn(detail) {
      ...
      this.$router.push({
        path:
          "/instorage_action/procurement_storage/stockIn/" + detail.InvoiceNo
      });
    },
    handleConfirm(detail) {
      ...
      this.$router.push({
        path:
          "/instorage_action/procurement_storage/confirm/" + detail.InvoiceNo
      });
    },
    getDoc(row) {
      let token = getToken();
      token = token.split(" ", 2);
      window.open(
        "/api/procurement_storage/exportDoc?id=" +
          row.InvoiceNo +
          "&token=" +
          token[1],
        "_blank"
    ...
    exportOut() {
      ...
      for (let i = 0; i < this.checked.length; i++) {
        goodsIdsList.push(this.checked[i].InvoiceNo);
      }
      window.open(
        "/api/procurement_storage/export?id=" +
          goodsIdsList+ "&query=" + JSON.stringify(this.query) +
          "&token=" +
          token[1],
        "_blank"
      );
