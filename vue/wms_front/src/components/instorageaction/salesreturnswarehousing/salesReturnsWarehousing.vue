<template>
  <div class="grid-container">
    ...
    <div class="grid-content">
      <el-table :data="tableData" v-loading.body="loading" @selection-change="handleSelectionChange" :row-key="getRowKeys" border stripe height="800" style="width: 100%">
        ...
        <el-table-column label="操作" width="150" class-name="action-column">
          <template slot-scope="scope">
            <div class="action-column">
              ...
              >确认</el-button>
              <el-button type="text" size="small" @click.native.prevent="showDetail(scope.row)">查看</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
...
<script>
import { fetchList } from "@/api/sales_return_instorage";
...
export default {
  ...
  created() {
    if (localStorage.salesreturn_query) {
      this.query = JSON.parse(localStorage.salesreturn_query);
      delete localStorage.salesreturn_query;
    }
  ...
  methods: {
    ...
    showDetail: function(detail) {
      localStorage.salesreturn_query = JSON.stringify(this.query);
      this.$router.push({
        path:
          "/instorage_action/sales_returns_warehousing/detail/" + detail.OrderNo
      });
    },
    getRowKeys(row) {
      return row.OrderNo;
    ...
    handleStockIn(detail) {
      ...
      this.$router.push({
        path:
          "/instorage_action/sales_returns_warehousing/stockIn/" +
          detail.OrderNo
      });
    },
    getDoc(row) {
      ...
      token = token.split(" ", 2);
      window.open(
        "/api/sales_return_instorage/exportDoc?id=" +
          row.OrderNo +
          "&token=" +
          token[1],
        "_blank"
    ...
    handleConfirm(detail) {
      ...
      this.$router.push({
        path:
          "/instorage_action/sales_returns_warehousing/confirm/" +
          detail.OrderNo
      });
    },
    exportOut() {
      ...
      window.open(
              "/api/sales_return_instorage/export?id=" + goodsIdsList + "&query=" + JSON.stringify(this.query)  + "&token=" + token[1],
              "_blank"
      );
      localStorage.salesreturn_query = JSON.stringify(this.query);
      this.loadData()
