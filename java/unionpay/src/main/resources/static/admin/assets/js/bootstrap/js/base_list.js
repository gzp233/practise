	...
	$.extend({
		table:{
			...
			oTableInit:function(parms){//初始化表单
				$.table._option=parms;
				var singleSelectToggle = false;
				if(parms.modalName === "用户"){  // 用户管理
                    singleSelectToggle = true; // 单选
                } else {
                    singleSelectToggle = false;  // 多选
                };
			    ...
			    oTableInit.Init = function () {
			        $('#dataTable').bootstrapTable({
                        ...
                        escape:true,
                        singleSelect: singleSelectToggle,
			           ...
			           // height: 460,            //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
			            ...
			            onLoadSuccess: function(){  //加载成功时执行
			                console.info("加载成功");
			                $.table.fn_success($.table.select_rows);
			                console.log($.table.select_rows)
				        },
			...
			fn_success: function(data){
				if(data.length > 0){
					var bscheckbox = $("#dataTable tbody tr");
					$.each(bscheckbox, function(index,item){
						for(var i=0;i<data.length;i++){
							if(data[i].id == $(item).data("uniqueid")){
								$(item).addClass("selected");
								$(item).children(".bs-checkbox").children("input").attr("checked","checked")
							};
						};
					});
				};
		...
		operate:{
			...
			remove:function(id, username){
                if(username === "interface_admin"){
                    $.modal.alertWarning("该用户禁止删除");
                    return;
                } else {
