...
import com.cpdms.util.SecurityUtil;
...
import com.cpdms.model.auto.SysNotice;
import com.cpdms.model.auto.TsysUser;
import com.cpdms.model.custom.BootstrapTree;
...
public class AdminController extends BaseController{
	...
	public String index(HttpServletRequest request) {
    	...
    	//获取公告信息
    	List<SysNotice>  notices=sysNoticeService.getuserNoticeNotRead(ShiroUtils.getUserId(),0);
	 ...
	 */
	@PostMapping("login")
	public ModelAndView login(String username,String password,String account,String pwd,String code,RedirectAttributes redirectAttributes,boolean rememberMe,HttpServletRequest request) throws Exception {
		...
		//logger.info("password dec"+ SecurityUtil.decryptToLogin(password,SecurityUtil.privateKey,SecurityUtil.pad));
		//logger.info("username dec" + SecurityUtil.decryptToLogin(username,SecurityUtil.privateKey,SecurityUtil.pad));
		//logger.info("code" + code);
		 ...
		 if(StringUtils.isNotEmpty(scode)&&StringUtils.isNotEmpty(code)&&scode.equals(code)){
			 ...
			 String des_password = SecurityUtil.decryptToLogin(password,SecurityUtil.privateKey,SecurityUtil.pad);
			 String des_username = SecurityUtil.decryptToLogin(username,SecurityUtil.privateKey,SecurityUtil.pad);
						 Subject currentUser = SecurityUtils.getSubject();
				 UsernamePasswordToken token =new UsernamePasswordToken(des_username,des_password);
				 try {
					 if(rememberMe) {
					 ...
					 currentUser.login(token);
			            ...
			            logger.info("对用户[" + des_username + "]进行登录验证..验证未通过,未知账户");
			            redirectAttributes.addFlashAttribute("message", "未知账户");
			        } catch (IncorrectCredentialsException ice) {
			            logger.info("对用户[" + des_username + "]进行登录验证..验证未通过,错误的凭证");
			            redirectAttributes.addFlashAttribute("message", "用户名或密码不正确");
			        } catch (LockedAccountException lae) {
			            logger.info("对用户[" + des_username + "]进行登录验证..验证未通过,账户已锁定");
			            redirectAttributes.addFlashAttribute("message", "账户已锁定");
			        } catch (ExcessiveAttemptsException eae) {
			            logger.info("对用户[" + des_username + "]进行登录验证..验证未通过,错误次数过多");
			            redirectAttributes.addFlashAttribute("message", "用户名或密码错误次数过多");
			        } catch (AuthenticationException ae) {
			            //通过处理Shiro的运行时AuthenticationException就可以控制用户登录失败或密码错误时的情景
			            logger.info("对用户[" + des_username + "]进行登录验证..验证未通过,堆栈轨迹如下");
			            ae.printStackTrace();
			            redirectAttributes.addFlashAttribute("message", "用户名或密码不正确");
			        }
